...
function create() {
	...
	// This case uses all parameters. The joint anchor is offset in each body.
	{
		...
		//bodyA, bodyB, length, ax, ay, bx, by, frequency, damping
		game.physics.box2d.distanceJoint(spriteA, spriteB, 150, -20, -60, 40, 40, 3, 0.25);
...
function update() {
	...
	if ( bodyAs[0].containsPoint(game.input.mousePointer) ) {
		...
		codeCaption.text = 'game.physics.box2d.distanceJoint(spriteA, spriteB, 200)';
	}
	else if ( bodyAs[2].containsPoint(game.input.mousePointer) ) {
		codeCaption.text = 'game.physics.box2d.distanceJoint(spriteA, spriteB, 200, -20, -60, 40, 40, 3, 0.25)';
	}
	else {
		codeCaption.text = '';
	}
