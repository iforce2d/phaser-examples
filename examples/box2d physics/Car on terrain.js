...
function create() {
	...
	driveJoints[0] = game.physics.box2d.wheelJoint(truckBody, wheelBodies[0], -0.82*PTM,wheelLift*PTM, 0,0, 0,1, frequency, damping, 0, motorTorque, true ); // rear
	driveJoints[1] = game.physics.box2d.wheelJoint(truckBody, wheelBodies[1],  1.05*PTM,wheelLift*PTM, 0,0, 0,1, frequency, damping, 0, motorTorque, true ); // front

	cursors = game.input.keyboard.createCursorKeys();
	
	game.camera.follow(truckBody);
	
	caption = game.add.text(5, 5, 'Simple car control. Left/right arrow keys to move, down arrow key to brake.', { fill: '#ffffff', font: '14pt Arial' });
	
}

function update() {
	
	// Is there a better way to stop the caption from moving with the world?
	// This method makes it jiggle around noticeably.
	caption.x = game.camera.x + 5;
	caption.y = game.camera.y + 5;
